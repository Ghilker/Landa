...
if ($_SESSION['permessi'] < ADMIN) {
	...
	<div class="page_title">
		...
		<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/gest_luoghi.png"></div>
	</div>
	...
	if ((gdrcd_filter('get', $_POST['op']) == 'edit') || (gdrcd_filter('get', $_REQUEST['op']) == 'new')) {
		...
		<div class="panels_box">
			<form action="main.php?page=gestione_luoghi" method="post" class="form_gestione">
				...
				<div class='form_field'>
					<textarea
						name="descrizione"><?php echo gdrcd_filter('out', $loaded_location['descrizione']); ?></textarea>
				</div>
				...
				<div class='form_field'>
					<input type="checkbox" name="chat" <?php if
						(($loaded_location['chat'] == 1) || (isset($loaded_location) === FALSE)) { ?>
					checked="checked"
					<?php } ?>
					value="is_chat" />
				</div>
		...
		if (gdrcd_query($mappe, 'num_rows') > 0) { ?>
					...
					<select name="mappa_linked">
						<!-- Opzione "Nessuna" -->
						<option value="0">
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['locations']['map_id_default']); ?>
						</option><!-- Opzione "Nessuno" -->
						<?php while ($option = gdrcd_query($mappe, 'fetch')) { ?>
						<option value="<?php echo gdrcd_filter('out', $option['id_click']); ?>" <?php
							if ($loaded_location['id_mappa_collegata'] == $option['id_click']) {
					echo 'SELECTED';
				} ?>>
		...
		if (gdrcd_query($mappe, 'num_rows') > 0) { ?>
					...
					<select name="mappa">
						...
						<?php while ($option = gdrcd_query($mappe, 'fetch')) { ?>
						<option value="<?php echo gdrcd_filter('out', $option['id_click']); ?>" <?php
							if ($loaded_location['id_mappa'] == $option['id_click']) {
					echo 'SELECTED';
				} ?>>
							<?php echo gdrcd_filter('out', $option['nome']); ?>
						</option>
			...
			echo gdrcd_filter('out', $MESSAGE['interface']['administration']['locations']['map_id_err']);
				...
				<div class='form_field'>
					<input type="checkbox" name="privata" <?php if ($loaded_location['privata'] == 1) { ?>
					checked="checked"
					<?php } ?>
					value="is_privat" />
				</div>
			...
			if (gdrcd_query($nomi, 'num_rows') > 0 || gdrcd_query($gilde, 'num_rows') > 0) { ?>
					...
					<select name="proprietario">
						<!-- Opzione "Nessuno" -->
						<option
							value="<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['locations']['owner_default']); ?>">
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['locations']['owner_default']); ?>
						</option>
						<!-- Gilde -->
						<optgroup label="<?php echo gdrcd_filter('out', $PARAMETERS['names']['guild_name']['plur']); ?>">
						</optgroup>
						<?php while ($option = gdrcd_query($gilde, 'fetch')) { ?>
						<option value="<?php echo gdrcd_filter('out', $option['id_gilda']); ?>" <?php if
							($option['id_gilda'] == $loaded_location['proprietario']) {
						echo " selected";
					} ?>>
							<?php echo gdrcd_filter('out', $option['nome']); ?>
						</option>
				...
				gdrcd_query($gilde, 'free');
                   ?>
						...
						<optgroup label="<?php echo gdrcd_filter('out', $PARAMETERS['names']['users_name']['plur']); ?>">
							...
							<option value="<?php echo gdrcd_filter('out', $option['nome']); ?>" <?php if
								($option['nome'] == $loaded_location['proprietario']) {
						echo " selected";
					} ?>>
...
<?php if ((isset($_POST['op']) === FALSE) && (isset($_REQUEST['op']) === FALSE)) { /*Elenco record (Visualizzaione di base della pagina)*/
	...
	<div class="elenco_record_gestione">
		<table>
			...
			<tr>
				...
				<td class="casella_titolo">
					<div class="titoli_elenco">
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['locations']['map_name']); ?>
					</div>
				</td>
				<td class="casella_titolo">
					<div class="titoli_elenco">
			...
			<tr>
				...
				<td class="casella_elemento">
					<div class="elementi_elenco">
						<?php echo gdrcd_filter('out', $row['mappa_click']); ?>
					</div>
				</td>
	...
	<a href="main.php?page=gestione_luoghi&offset=<?php echo $i; ?>">
		<?php echo $i + 1; ?>
	</a>
	<?php } else {
					echo ' ' . ($i + 1) . ' ';
