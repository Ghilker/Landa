...
?>
		...
		<div class="link_back">
			<a href="main.php?page=messages_center&offset=0">
				<?php echo $MESSAGE['interface']['messages']['go_back']; ?>
			</a>
		</div>
		<?php
} //if 
...
?>
	...
	if (isset($_POST['testo']))
		echo "\n\n\n[" . gdrcd_filter('out', trim($_POST['testo'])) . "]";
               ...
               ?></textarea>
				...
				<input type="hidden" name="reply_attach"
		...
		<div class="link_back">
			<a href="main.php?page=messages_center&offset=0">
				<?php echo gdrcd_filter('out', $MESSAGE['interface']['messages']['go_back']); ?>
			</a>
		</div>
		<?php } //if ?>
 ...
 ?>
		<div class="warning">
			Impossibile visualizzare il messaggio richiesto, il messaggio potrebbe non esistere oppure non disponi delle
			autorizzazioni necessarie per poterlo visionare
		</div>
		<?php
	 } else {
	...
	<div class="infos">
		...
		<span class="body">
			<?php echo gdrcd_filter('out', $record['mittente']); ?>
		</span>
	</div>
	...
	<div class="read_message_box_forms">
		...
		<div class="read_message_box_form">
			...
			<form action="main.php?page=messages_center" method="post">
				...
				<input type="image" style="background:transparent; border-style:none" ; src="imgs/icons/attach.png"
					value="submit" alt="<?php echo gdrcd_filter('out', $MESSAGE['interface']['messages']['attach']); ?>"
		...
		<div class="read_message_box_form">
			...
			<form action="main.php?page=messages_center" method="post">
				...
				<input type="image" style="background:transparent; border-style:none" ; src="imgs/icons/reply.png"
					value="submit" alt="<?php echo gdrcd_filter('out', $MESSAGE['interface']['messages']['reply']); ?>"
...
<div class="link_back" style="margin-left:18%">
	<a href="main.php?page=messages_center&offset=0">
		<?php echo gdrcd_filter('out', $MESSAGE['interface']['messages']['go_back']); ?>
	</a>
...
//ORIGINALE: gdrcd_query("DELETE FROM messaggi WHERE id = ".$id_messaggio." and (destinatario = '". $_SESSION['login'] ."' OR mittente = '". $_SESSION['login'] ."') LIMIT 1");
...
if ($_POST['op'] == 'erase_checked') {
	if (!empty($_POST['ids'])) {
		foreach ($_POST['ids'] as $k => $v) {
			...
			/*else{
			unset($_POST['ids'][$k]);
			} --> FANCULO ERA QUESTO CHE BUGGAVA TUTTO*/
		}
		$msgs = implode(',', $_POST['ids']);
		//ORIGINALE: $query="DELETE FROM messaggi WHERE (destinatario='".gdrcd_filter('in', $_SESSION['login'])."' OR mittente='".gdrcd_filter('in', $_SESSION['login'])."') AND id IN (".$msgs.")";
//$query="DELETE FROM messaggi WHERE destinatario='".gdrcd_filter('in', $_SESSION['login'])."' AND id IN (".$msgs.")";
//gdrcd_query($query);

		gdrcd_query("DELETE FROM messaggi WHERE (destinatario='" . gdrcd_filter('in', $_SESSION['login']) . "' OR mittente='" . gdrcd_filter('in', $_SESSION['login']) . "') AND id IN (" . $msgs . ")");
...
<div class="elenco_record_gioco">
			...
			<td>
				<div class="elementi_elenco">
					<a href="main.php?page=messages_center&op=read&id_messaggio=<?php echo $row['id'] ?>">
						<?php echo gdrcd_filter('out', substr($row['testo'], 0, 40)); ?>...
					</a>
				</div>
			</td>
			<td>
				...
				<div class="controlli_elenco">
					...
					<div class="controllo_elenco">
						...
						<form action="main.php?page=messages_center" method="post">
							...
							<input type="submit"
								value="<?php echo gdrcd_filter('out', $MESSAGE['interface']['messages']['reply']); ?>" />
 ...
 ?>
	<div class="pager">
		...
		<a href="main.php?page=messages_center&offset=<?php echo $i; ?>">
			<?php echo $i + 1; ?>
		</a>
		<?php } else {
				 echo ' ' . ($i + 1) . ' ';
