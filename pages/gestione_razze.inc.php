...
if ($_SESSION['permessi'] < GESTORE) {
	...
	<div class="page_title">
		...
		<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/gest_razze.png"></div>
	</div>
	...
	if ($_POST['op'] == 'insert') {
		...
		if ($_POST['tripla'] == "") {
			$tripla = "standard_razza.png";
		} else {
			$tripla = gdrcd_filter('in', $_POST['tripla']);
		}
		/*Eseguo l'inserimento*/
		gdrcd_query("INSERT INTO razza (nome_razza, sing_m, sing_f, descrizione, visibile, iscrizione, immagine, icon, tripla, bonus_car0, bonus_car1, bonus_car2, bonus_car3, bonus_car4, bonus_car5, url_site) VALUES ('" . gdrcd_filter('in', $_POST['nome']) . "', '" . gdrcd_filter('in', $_POST['sing_m']) . "', '" . gdrcd_filter('in', $_POST['sing_f']) . "', '" . gdrcd_filter('in', $_POST['descrizione']) . "', " . $is_visible . ", " . $is_available . ", '" . $immagine . "', '" . $icon . "','" . $tripla . "', " . gdrcd_filter('num', $_POST['car0']) . ", " . gdrcd_filter('num', $_POST['car1']) . ", " . gdrcd_filter('num', $_POST['car2']) . ", " . gdrcd_filter('num', $_POST['car3']) . ", " . gdrcd_filter('num', $_POST['car4']) . ", " . gdrcd_filter('num', $_POST['car5']) . ", '" . gdrcd_filter('in', $_POST['url_site']) . "')");
		...
		<div class="panels_box">
			<div class="form_gestione">
				<form action="main.php?page=gestione_razze" method="post">
					...
					<div class='form_field'>
						<input type="checkbox" name="visible" <?php if ($loaded_record['visibile'] == 1) { ?>
						checked="checked"
						<?php } ?>
						value="is_visible" />
					</div>
					...
					<div class='form_field'>
						<input type="checkbox" name="available" <?php if ($loaded_record['iscrizione'] == 1) { ?>
						checked="checked"
						<?php } ?>
						value="is_available" />
					</div>
					...
					<div class='form_label'>
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['races']['tripla']); ?>
					</div>
					<div class='form_field'>
						<input name="tripla" value="<?php echo $loaded_record['tripla']; ?>" />
					</div>
...
<?php if ((isset($_POST['op']) === FALSE) && (isset($_REQUEST['op']) === FALSE)) { /*Elenco record (Visualizzaione di base della pagina)*/
				...
				<tr>
					<td class="casella_titolo">
						<div class="titoli_elenco">
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['name_col']); ?>
						</div>
					</td>
					<td class="casella_titolo">
						<div class="titoli_elenco">
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['races']['avalaible']); ?>
						</div>
					</td>
					<td class="casella_titolo">
						<div class="titoli_elenco">
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['races']['visible']); ?>
						</div>
					</td>
					<td class="casella_titolo">
						<div class="titoli_elenco">
				...
				<tr class="risultati_elenco_record_gestione">
					<td class="casella_elemento">
						<div class="elementi_elenco">
							<?php echo $row['nome_razza']; ?>
						</div>
					</td>
					<td class="casella_elemento">
						<div class="elementi_elenco">
							<?php if ($row['iscrizione'] == 1) {
					...
					<td class="casella_controlli">
						<!-- Iconcine dei controlli -->
						<!-- Modifica -->
						<div class="controlli_elenco">
							<div class="controllo_elenco">
								<form class="opzioni_elenco_record_gestione" action="main.php?page=gestione_razze"
									method="post">
	 ...
	 <div class="pager">
       <?php if ($totaleresults > $PARAMETERS['settings']['records_per_page']) {
		...
		<a href="main.php?page=gestione_razze&offset=<?php echo $i; ?>">
			<?php echo $i + 1; ?>
		</a>
		<?php } else {
					echo ' ' . ($i + 1) . ' ';
