<div class="pagina_gestione_mercato">
	<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/gest_oggetti.png"></div>
					...
					<?php if (gdrcd_query($tipi_oggetto, 'num_rows') > 0) { ?>
						<select name="tipo_oggetto">
							<?php while ($option = gdrcd_query($tipi_oggetto, 'fetch')) { ?>
								<option value="<?php echo $option['cod_tipo']; ?>" <?php if ($loaded_item['tipo'] == $option['cod_tipo']) {
								   echo 'SELECTED';
							   } ?>>
							...
							<?php if (gdrcd_query($characters, 'num_rows') > 0) { ?>
								<select name="give_item">
									<option value="mercato">
										<?php echo gdrcd_filter('out', $PARAMETERS['names']['market_name']); ?>
									</option>
