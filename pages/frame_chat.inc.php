...
<div class="pagina_frame_chat">
	...
	<div class="page_title">
		<h2><img src="imgs/cerchisx.png" height="10px">
			<?php echo $info['nome']; ?><img src="imgs/cerchidx.png" height="10px">
		</h2>
	</div>
...
if ($info['privata'] == 1) {
	...
	if (
		(($info['proprietario'] == gdrcd_capital_letter($_SESSION['login'])) || (strpos($_SESSION['gilda'], $info['proprietario']) != FALSE) || (strpos($info['invitati'], gdrcd_capital_letter($_SESSION['login'])) != FALSE) ||
			(($PARAMETERS['mode']['spyprivaterooms'] == 'ON') && ($_SESSION['permessi'] > ADMIN))) && ($info['scadenza'] > strftime('%Y-%m-%d %H:%M:%S'))
	) {
		$allowance = TRUE;
	}
...
?>
	   ...
	   <?php if ($_SESSION['permessi'] == MASTER) { ?> <!--PRIMA ERA >=MASTER-->
								<option value="2">
									<?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][2]); //master ?></option>  
	   <option value="3"><?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][3]); //png ?></option>
	   <?php } ?>
									<?php if ($_SESSION['permessi'] == ADMIN) { ?>
									<!--STRINGA AGGIUNTA DA ME-->
								<option value="2">
									<?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][2]); //master ?></option>  
	   <option value="3"><?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][3]); //png ?></option>
	   <?php } ?>
									<?php if ($_SESSION['permessi'] == GESTORE) { ?>
									<!--STRINGA AGGIUNTA DA ME-->
								<option value="2">
									<?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][2]); //master ?></option>  
	   <option value="3"><?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][3]); //png ?></option>
	   <?php } ?>
								...
								<option value="5">
									<?php echo gdrcd_filter('out', $MESSAGE['chat']['type'][5]); //invita ?></option>
	...
	<br/><div class="output_testo"><?php echo gdrcd_filter('out', $MESSAGE['chat']['type']['info']); ?>
						</div>
				</div>
	...
	if ($_SESSION['permessi'] >= MASTER) {
		echo gdrcd_filter('out', $MESSAGE['chat']['tag']['info']['png']);
			...
			<div class="form_row">
				<form action="pages/chat.inc.php?ref=30&chat=yes" method="post" target="chat_frame"
					...
					<!--DISABILITA ABILITA' E CARATTERISTICHE erano OFF ON OFF-->
       ...
       ?>
			...
			*/
            ...
            ?>
							<option value="stats_<?php echo substr($id_stats, 3); ?>">
								<?php echo $name_stats; ?>
							</option>
							<?php
...
?>
							<option value="<?php echo $dice_value; ?>">
								<?php echo $dice_name; ?>
							</option>
							<?php
			}
?>
			...
			echo '<input type="hidden" name="dice" id="dice" value="no_dice">';
					...
					<?php if ($PARAMETERS['mode']['skillsystem'] == 'ON') { ?>
					<!--DISABILITA USO OGGETTO era OFF-->
