...
if (
	(isset($_POST['op']) === TRUE) &&
	($_POST['op'] == gdrcd_filter(
		'out',
		$MESSAGE['interface']['maps']['set_meteo']
	)
	)
) {
	/*Aggiorno la sua posizione*/
...
<div class="mappaclick_more">
	<?php /* Carico le mappe dell'eventuale vicinato */
		$result = gdrcd_query("SELECT id_click, nome FROM mappa_click WHERE posizione = " . $vicinato . " AND id_click <> " . $self . " ORDER BY id_click", 'result');
		if (gdrcd_query($result, 'num_rows') > 0) {
...
<div class="form_box">
	...
	<form class="form_gioco" action="main.php?page=mappaclick&map_id=<?php echo $_SESSION['mappa']; ?>" method="post">
		...
		<div class="form_submit">
			...
			<input type="submit" value="<?php echo gdrcd_filter('out', $MESSAGE['interface']['maps']['leave']); ?>""
			   name=" op" />
		</div>
		<?php } else {
...
<?php }
		if ($PARAMETERS['mode']['auto_meteo'] == 'OFF') { ?>
			<form class="form_gioco" action="main.php?page=mappaclick&map_id=<?php echo $_SESSION['mappa']; ?>"
				method="post">
				<div class="form_submit">
