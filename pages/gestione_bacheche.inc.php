...
if ($_SESSION['permessi'] < ADMIN) {
	...
	<div class="page_title">
		...
		<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/gest_bacheche.png"></div>
	</div>
...
?>
		...
		<div class="panels_box">
			<form action="main.php?page=gestione_bacheche" method="post" class="form_gestione">
				...
				<div class='form_field'>
					<!-- Elenco dei tipi -->
					<select name="tipo">
						<option value="<?php echo INGIOCO; ?>" <?php if ($loaded_record['tipo'] == INGIOCO) {
			echo
							"selected";
		} ?>>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][INGIOCO]); ?>
						</option>
						<option value="<?php echo PERTUTTI; ?>" <?php if ($loaded_record['tipo'] == PERTUTTI) {
			echo
							"selected";
		} ?>>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][PERTUTTI]); ?>
						</option>
						<option value="<?php echo SOLORAZZA; ?>" <?php if ($loaded_record['tipo'] == SOLORAZZA) {
			echo
							"selected";
		} ?>>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLORAZZA]); ?>
						</option>
						<option value="<?php echo SOLOGILDA; ?>" <?php if ($loaded_record['tipo'] == SOLOGILDA) {
			echo
							"selected";
		} ?>>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOGILDA]); ?>
						</option>
						<option value="<?php echo SOLOMASTERS; ?>" <?php if ($loaded_record['tipo'] == SOLOMASTERS) {
			echo
							"selected";
		} ?>>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['type'][SOLOMASTERS]); ?>
						</option>
						<option value="<?php echo SOLOMODERATORS; ?>" <?php if
							($loaded_record['tipo'] == SOLOMODERATORS) {
			echo "selected";
		} ?>>
		...
		$gilde = gdrcd_query("SELECT id_gilda, nome FROM gilda", 'result'); ?>
					...
					<select name="owner">
						...
						<option value="<?php echo gdrcd_filter('out', $option['id_razza']); ?>" <?php
							if (($loaded_record['proprietari'] == $option['id_razza']) &&
							($loaded_record['tipo'] == SOLORAZZA)) {
				echo 'SELECTED';
			} ?>>
		...
		gdrcd_query($razze, 'free');
                 ?>
						...
						<option value="<?php echo gdrcd_filter('out', $option['id_gilda']); ?>" <?php
							if (($loaded_record['proprietari'] == $option['id_gilda']) &&
							($loaded_record['tipo'] == SOLOGILDA)) {
				echo 'SELECTED';
			} ?>>
