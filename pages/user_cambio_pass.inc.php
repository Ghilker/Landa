...
<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/cambio_pw.png"></div>
</div>
...
if(gdrcd_filter('get',$_POST['op'])=='force'){
	...
	if (($_SESSION['permessi']>=ADMIN)&&(gdrcd_check_pass($_POST['new_pass'])===TRUE)){
         if ($_SESSION['permessi']==GESTORE){ 
             ...
             $query="UPDATE personaggio SET pass = '".gdrcd_encript($_POST['new_pass'])."', ultimo_cambiopass = NOW() WHERE nome = '".gdrcd_filter_in($_POST['account'])."' AND permessi < ".GESTORE.""; 
		 }
         gdrcd_query($query);
