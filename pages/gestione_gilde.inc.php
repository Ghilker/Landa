...
if ($_SESSION['permessi'] < ADMIN) {
	...
	<div class="page_title">
		...
		<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/gest_gilde.png"></div>
	</div>
	...
	if (gdrcd_filter('get', $_POST['op']) == 'nuovo_ruolo') {
		...
		gdrcd_query("INSERT INTO ruolo (nome_ruolo, gilda, immagine, stipendio, descrizione, capo) VALUES ('" . gdrcd_filter('in', $_POST['nome']) . "', " . gdrcd_filter('num', $_POST['gilda']) . ", '" . gdrcd_filter('in', $immagine) . "', '" . gdrcd_filter('num', $_POST['stipendio']) . "', '" . gdrcd_filter('in', $_POST['descrizione']) . "', '" . $is_capo . "')");
?>
		<!-- Conferma -->
		<div class="warning">
...
?>
	...
	if (
		...
		gdrcd_query("UPDATE ruolo SET nome_ruolo ='" . gdrcd_filter('in', $_POST['nome']) . "', capo = " . $is_capo . ", immagine = '" . gdrcd_filter('in', $immagine) . "', gilda = " . gdrcd_filter('num', $_POST['gilda']) . ", stipendio = " . gdrcd_filter('num', $_POST['stipendio']) . ", descrizione = '" . gdrcd_filter('in', $_POST['descrizione']) . "' WHERE id_ruolo = " . gdrcd_filter('num', $_POST['id_ruolo']) . " LIMIT 1");
?>
		<!-- Conferma -->
		<div class="warning">
			<?php echo gdrcd_filter('out', $MESSAGE['warning']['modified']); ?>
			...
			if (gdrcd_query($tipi, 'num_rows') > 0) { ?>
					...
					<select name="tipo">
						...
						<option value="<?php echo $option['cod_tipo']; ?>" <?php
							if ($loaded_record['tipo'] == $option['cod_tipo']) {
						echo 'SELECTED';
					} ?>>
				...
				<div class='form_field'><textarea
						name="statuto"><?php echo gdrcd_filter('out', $loaded_record['statuto']); ?></textarea>
				</div>
				...
				<div class='form_field'>
					<input type="checkbox" name="visible" <?php if (gdrcd_filter('out', $loaded_record['visibile']) == 1)
						{ ?>
					checked="checked"
					<?php } ?>
					value="is_visible" />
...
<?php if (
		...
		<div class="page_title">
			<h2>
				<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['guilds']['role']['page_name']); ?>
			</h2>
		</div>
		...
		<div class="page_body">
			...
			<form action="main.php?page=gestione_gilde" method="post" class="form_gestione">
				...
				<div class='form_field'>
					<input name="descrizione" value="Inserisci descrizione lavoro..." />
				</div>
			...
			<form action="main.php?page=gestione_gilde" method="post" class="form_gestione">
				<div class="elenco_record_gestione">
					<table>
						...
						<!-- AGGIUNTA DESCRIZIONE LAVORI NEL FORM DI MODIFICA-->

						<tr>
							<td colspan="3">
								<div class='titoli_elenco'>
									<?php echo "DESCRIZIONE"; /*gdrcd_filter('out',$MESSAGE['interface']['administration']['guilds']['role']['descrizione']); */?>
								</div>
							</td>
						</tr>
						<tr>
								...
								<div class='form_field'>
									<form action="traitement_cmd.php" method="post">
										<textarea rows="5" cols="80"
											name="descrizione"><?php echo gdrcd_filter('out', $row['descrizione']); ?></textarea>
									</form>
								</div>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<?php if (gdrcd_filter('get', $_REQUEST['id_record'] > -1)) { ?>
								<div class='form_field'>
									<input type="checkbox" name="capo" <?php if ($row['capo'] == 1) {
						echo 'checked';
					} ?>
									value="is_capo" />
								</div>
							...
							<td>
								<div class='form_submit'>
									...
									<input type="hidden" name="id_ruolo"
										value="<?php echo gdrcd_filter('out', $row['id_ruolo']); ?>" />
									<input type="hidden" name="gilda"
...
<?php if (
				...
				<td class="casella_controlli">
					...
					<div class="controlli_elenco">
						<div class="controllo_elenco">
							<form class="opzioni_elenco_record_gestione" action="main.php?page=gestione_gilde"
								method="post">
								<input type="hidden" name="id_record"
	...
	<a href="main.php?page=gestione_gilde&offset=<?php echo $i; ?>">
		<?php echo $i + 1; ?>
	</a>
	<?php } else {
					echo ' ' . ($i + 1) . ' ';
