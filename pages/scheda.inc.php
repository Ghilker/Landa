...
if (isset($_REQUEST['pg']) === FALSE) {
	...
	<div class="panels_box">
		<div class="form_gioco">
			<form action="main.php?page=scheda_modifica&pg=<?php echo gdrcd_filter('url', $_REQUEST['pg']) ?>"
				method="post">
				...
				<input type="hidden"
					value="<?php gdrcd_filter('out', $MESSAGE['interface']['sheet']['modify_form']['unexile']); ?>"
					name="causale" />
	...
	<?php }
			...
			/*Incremento skill*/
			if ((gdrcd_filter('get', $_REQUEST['op']) == 'addskill') && (($_SESSION['login'] == gdrcd_filter('out', $_REQUEST['pg'])) || ($_SESSION['permessi'] >= CAPORAZZA))) {
			...
			/*Decremento skill*/
			if ((gdrcd_filter('get', $_REQUEST['op']) == 'subskill') && ($_SESSION['permessi'] >= CAPORAZZA)) {
...
?>
		...
		<div class="menu_scheda">
			...
			<?php /*Visualizza il link modifica se l'utente visualizza la propria scheda o se è almeno un capogilda*/
				if ($_REQUEST['pg'] == $_SESSION['login'] || $_SESSION['permessi'] >= MASTER) { ?>
     ...
     <?php echo gdrcd_filter('out', $MESSAGE['interface']['sheet']['box_title']['portrait']); ?>
			...
			<div class="ritratto_invia_messaggio">
				<!-- Link invia messaggio -->
				<a href="main.php?page=messages_center&newmessage=yes&reply_dest=<?php echo gdrcd_filter('url', $record['nome']); ?>"
  ...
  <div class="profilo_voce">
	 ...
	 <div class="profilo_voce_valore">
	    <?php echo gdrcd_filter('out', $record['car5'] + $record['bonus_car5'] + $bonus_oggetti['BO5']); ?>
			...
			<!--SECONDA TABELLA AGGIUNTA DA ME-->
			<div class="profilo_status2">
				<div class="profilo_status2_label">
					<?php echo gdrcd_filter('out', $MESSAGE['interface']['sheet']['profile']['status2']); ?>:
				</div>
				<div class="profilo_status2_valore">
					<?php echo nl2br(gdrcd_filter('out', $record['stato2'])); ?>
				</div>
			</div>
			...
			<tr>
				...
				<td>
					<div class="abilita_scheda_sub">
						...
						if (
							(((($ranks[$row['id_abilita']] + 1) * $PARAMETERS['settings']['px_x_rank']) <= ($px_totali_pg - $px_spesi)) &&
								(gdrcd_filter('get',
									$_REQUEST['pg']) == $_SESSION['login']) &&
								($ranks[$row['id_abilita']] < $PARAMETERS['settings']['skills_cap'])) ||
							($_SESSION['permessi'] >= MASTER)
						) { ?>
						[<a
							href="main.php?page=scheda&pg=<?php echo gdrcd_filter('url', $_REQUEST['pg']) ?>&op=addskill&what=<?php echo $row['id_abilita'] ?>">+</a>]
						<?php if (
								($_SESSION['permessi'] >= MASTER) &&
								($ranks[$row['id_abilita']] > 0)
							) { ?>
						[<a
							href="main.php?page=scheda&pg=<?php echo gdrcd_filter('url', $_REQUEST['pg']) ?>&op=subskill&what=<?php echo $row['id_abilita'] ?>">-</a>]
						<?php } ?>
						<?php } else {
							echo '&nbsp;';
						} ?>
...
<!--- AREA ADMIN --->
<?php if ($_SESSION['permessi'] >= MASTER) { ?>
...
<object data="<?php echo $record['url_media']; ?>"
	type="<?php echo $PARAMETERS['settings']['audiotype']['.' . strtolower(end(explode('.', $record['url_media'])))]; ?>"
	autostart="true">
