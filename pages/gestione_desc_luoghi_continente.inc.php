<div class="pagina_gestione_abilita">
	...
	if ($_SESSION['permessi'] < GESTORE) {
		echo '<div class="error">' . gdrcd_filter('out', $MESSAGE['error']['not_allowed']) . '</div>';
	} else { ?>

		<!-- Titolo della pagina -->
		<div class="page_title">
			<!--<h2><?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['page_name']); ?></h2>-->
			<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/gest_ambientazione.png"></div>
		</div>
		...
		<div class="page_body">
			...
			if ($_POST['op'] == 'insert') {
				...
				gdrcd_query("INSERT INTO luoghi_continente (capitolo, titolo, testo) VALUES (" . gdrcd_filter('num', $_POST['articolo']) . ", '" . gdrcd_filter('in', $_POST['titolo']) . "', '" . gdrcd_filter('in', $_POST['testo']) . "')");
				?>
				<div class="warning">
				...
				<div class="link_back">
					<a href="main.php?page=gestione_luoghi_continente">
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['link']['back']); ?>
					</a>
				</div>
				<?php } ?>
			...
			if ($_POST['op'] == 'erase') {
				...
				gdrcd_query("DELETE FROM luoghi_continente WHERE capitolo=" . gdrcd_filter('num', $_POST['id_record']) . " LIMIT 1");
				?>
				<div class="warning">
			...
			if (gdrcd_filter('get', $_POST['op']) == 'doedit') {
				...
				gdrcd_query("UPDATE luoghi_continente SET titolo ='" . gdrcd_filter('in', $_POST['titolo']) . "', testo ='" . gdrcd_filter('in', $_POST['testo']) . "', capitolo = " . gdrcd_filter('in', $_POST['articolo']) . " WHERE capitolo = " . gdrcd_filter('num', $_POST['art']) . " LIMIT 1");
				?>
				<div class="warning">
			...
			if (
				...
				if ($_POST['op'] == 'edit') {
					...
					/*
					$query="SELECT * FROM luoghi_continente WHERE capitolo=".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1 ";
					$result=mysql_query($query);
					*/
					$loaded_record = gdrcd_query("SELECT * FROM luoghi_continente WHERE capitolo=" . gdrcd_filter('num', $_POST['id_record']) . " LIMIT 1 ");
					/*Cambio l'operazione in modifica*/
					$operation = 'edit';
				} ?>
				<!-- Form di inserimento/modifica -->
				<div class="panels_box">
					<form action="main.php?page=gestione_luoghi_continente" method="post" class="form_gestione">

						<div class='form_label'>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['art']); ?>
						</div>
						...
						<div class='form_label'>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['title']); ?>
						</div>
						<div class='form_field'>
						...
						<div class='form_label'>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['infos']); ?>
						</div>
						<div class='form_field'>
			...
			$record_globale = gdrcd_query("SELECT COUNT(*) FROM luoghi_continente");
			$totaleresults = $record_globale['COUNT(*)'];
			//Lettura record
			$result = gdrcd_query("SELECT capitolo, titolo, testo FROM luoghi_continente ORDER BY capitolo LIMIT " . $pagebegin . ", " . $pageend . "", 'result');
			...
			if ($numresults > 0) { ?>
					...
					<div class="elenco_record_gestione">
						<table>
							...
							<tr>
								...
								<td class="casella_titolo">
									<div class="titoli_elenco"><?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['titolo']); ?></div>
								</td>
								<td class="casella_titolo">
									<div class="titoli_elenco">
										...
										<div class="controlli_elenco">
											<div class="controllo_elenco">
												<form class="opzioni_elenco_record_gestione"
													action="main.php?page=gestione_luoghi_continente" method="post">
				...
				<div class="pager">
					...
					for ($i = 0; $i <= floor($totaleresults / $PARAMETERS['settings']['records_per_page']); $i++) {
						if ($i != gdrcd_filter('num', $_REQUEST['offset'])) { ?>
								<a href="main.php?page=gestione_luoghi_continente&offset=<?php echo $i; ?>">
									<?php echo $i + 1; ?>
								</a>
								<?php } else {
							echo ' ' . ($i + 1) . ' ';
				...
				<!-- link crea nuovo -->
				<div class="link_back">
					<a href="main.php?page=gestione_luoghi_continente&op=new">
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['desc_luoghi']['link']['new']); ?>
					</a>
				</div>
