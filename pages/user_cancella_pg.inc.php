...
<div class="pagina_user_cancella_pg">
	...
	<div class="page_title">
		...
		<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/cancella_account.png"></div>
	</div>
...
<div class="link_back">
	...
	<?php /*Cancella altri*/
if ((gdrcd_filter('get', $_POST['op']) == 'force') && ($_SESSION['permessi'] == ADMIN)) {
	gdrcd_query("UPDATE personaggio SET permessi = -1 WHERE nome = '" . gdrcd_filter('in', $_POST['account']) . "' AND permessi < " . GESTORE . "");
...
if ((gdrcd_filter('get', $_POST['op']) == 'force') && ($_SESSION['permessi'] == GESTORE)) {
...
?>
	...
	<?php /*Cambio pass utenti*/
if ((gdrcd_filter('get', $_POST['op']) == 'get_back') && ($_SESSION['permessi'] >= ADMIN)) {
				...
				<div class="form_field">
					<select name="account">
						<option disabled selected>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['user']['delete']['who']); ?>
						</option>
		...
		<div class="form_gioco">
			<form action="main.php?page=user_cancella_pg" method="post">
				...
				<div class="form_field">
					<select name="account">
						<option disabled selected>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['user']['get_back']['who']); ?>
						</option>
						<?php while ($row = gdrcd_query($result, 'fetch')) { ?>
						<option value="<?php echo $row['nome']; ?>">
							<?php echo $row['nome']; ?>
