...
<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/cambio_nome.png"></div>
</div>
...
if(gdrcd_filter('get',$_POST['op'])=='force'){
	...
	if (($_SESSION['permessi']>=ADMIN) && (empty($_POST['new_name'])===FALSE)){
		...
		gdrcd_query("UPDATE log SET autore = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE autore = '".$_POST['account']."'");
             	gdrcd_query("UPDATE messaggi SET mittente = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE mittente = '".$_POST['account']."' AND permessi < ".GESTORE."");
		gdrcd_query("UPDATE messaggi SET destinatario = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE destinatario = '".$_POST['account']."' AND permessi < ".ADMIN."");
             	gdrcd_query("UPDATE backmessaggi SET mittente = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE mittente = '".$_POST['account']."' AND permessi < ".GESTORE."");
		gdrcd_query("UPDATE backmessaggi SET destinatario = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE destinatario = '".$_POST['account']."' AND permessi < ".GESTORE."");
		gdrcd_query("UPDATE clgpersonaggioabilita SET nome = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE nome = '".$_POST['account']."' AND permessi < ".GESTORE."");
   		gdrcd_query("UPDATE clgpersonaggiomostrine SET nome = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE nome = '".$_POST['account']."' AND permessi < ".GESTORE."");
		gdrcd_query("UPDATE clgpersonaggiooggetto SET nome = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE nome = '".$_POST['account']."' AND permessi < ".GESTORE."");
   		gdrcd_query("UPDATE clgpersonaggioruolo SET personaggio = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE personaggio = '".$_POST['account']."' AND permessi < ".GESTORE."");
		gdrcd_query("UPDATE personaggio SET nome = '".gdrcd_filter('in',$_POST['new_name'])."' WHERE nome = '".gdrcd_filter('in',$_POST['account'])."' AND permessi < ".GESTORE.""); 
	}
...
<?php if ($_SESSION['permessi']>=ADMIN){
         if ($_SESSION['permessi']==GESTORE){ $query="SELECT nome FROM personaggio ORDER BY nome"; }
		 else { $query="SELECT nome FROM personaggio WHERE permessi < ".GESTORE." ORDER BY nome"; }
		 $result=gdrcd_query($query, 'result'); ?>
