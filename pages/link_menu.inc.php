  ...
  <select id="gotomap" onchange="self.location.href=this.value;">
    ...
    <?php foreach ($gotomap_list as $infoMap => $infoLocation) {
        $splitInfoMap = explode('|@|', $infoMap);
          ?>
    <option value="main.php?page=mappaclick&map_id=<?php echo $splitInfoMap[1]; ?>" <?php echo
      ($_SESSION['mappa']==$splitInfoMap[1] && $_SESSION['luogo']==-1) ? ' selected="selected"' : ''; ?>
      class="map">
      <?php echo $splitInfoMap[0]; ?>
    </option>
    ...
    <?php
        if (is_array($infoLocation)) {
          foreach ($infoLocation as $idLoc => $infoLoc) {
            ...
            ?>
    <option value="main.php?<?php echo $valueLoc; ?>" <?php echo ($_SESSION['luogo']==$idLoc && $_SESSION['luogo'] !=-1)
      ? ' selected="selected"' : ''; ?>>&raquo;
      <?php echo $infoLoc['nome']; ?>
    </option>
...
foreach ($PARAMETERS[$mkey] as $key => $link_menu) {
  ...
  if (!empty($link_menu['url'])) {
    ...
    if (empty($link_menu['image_file'])) {
      ...
      $content = 'style="background-image: url(themes/' . $PARAMETERS['themes']['current_theme'] . '/imgs/' . $mkey . '/' . $link_menu['image_file'] . ')" alt="' . gdrcd_filter(
        'out',
        $link_menu['text']
      ...
      $content = ' onMouseOver="n' . $mkey . $raw_counter . '_over_button()" onMouseOut="n' . $mkey . $raw_counter . '_up_button()"><img src= "themes/' . $PARAMETERS['themes']['current_theme'] . '/imgs/' . $mkey . '/' . $link_menu['image_file'] . '" alt="' . gdrcd_filter(
        'out',
        $link_menu['text']
      ) . '" title="' . gdrcd_filter(
          'out',
          $link_menu['text']
        ) . '" name="nmenu' . $raw_counter . '_buttonOne" />';
    ...
    echo '<div class="link_menu"><a href="' . $link_menu['url'] . '" style="" id="link_' . $mkey . '_' . $key . '"';
...
le istruzioni su come usare e configurare i menù sono riportate nel file config.inc.php */?>
  ...
  <div class="link_menu">
    <a href="main.php?dir=-1" style="margin-top: 0px; margin-left: 30px; display: inline; float: left;"
      id="link_menu_refresh" onmouseover="nmenu6_over_button()" onmouseout="nmenu6_up_button()"><img
        src="themes/tok1/imgs/menu/aggiorna.png" alt="Aggiorna" title="Aggiorna" name="nmenu6_buttonOne"></a>
    <a href="logout.php" style="margin-top: 5px; margin-left: 135px" id="link_menu_quit"
      onmouseover="nmenu6_over_button()" onmouseout="nmenu6_up_button()"><img src="themes/tok1/imgs/menu/esci.png"
        alt="Esci" title="Esci" name="nmenu6_buttonOne"></a>
  </div>
