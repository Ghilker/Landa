...
if ($_POST['op'] == 'insert') {
	...
	if (gdrcd_query($thread, 'num_rows')) {
		...
		if (
			($araldoData['tipo'] == SOLORAZZA and ($_SESSION['id_razza'] == $araldoData['proprietari'] || $_SESSION['permessi'] >= CAPORAZZA)) ||
			($araldoData['tipo'] == SOLOGILDA and (strpos($_SESSION['gilda'],
				'*' . $araldoData['proprietari'] . '*') !== FALSE || $_SESSION['permessi'] >= CAPOGILDA)) ||
			($araldoData['tipo'] >= SOLOMASTERS and $_SESSION['permessi'] >= MASTER) ||
			($araldoData['tipo'] >= SOLOMODERATORS and $_SESSION['permessi'] >= ADMIN) ||
			($araldoData['tipo'] == PERTUTTI) || ($araldoData['tipo'] == INGIOCO) || //Controllo Accesso al thread
			$_POST['padre'] == -1 or ($araldoData['chiuso'] != 1 || $_SESSION['permessi'] >= MASTER)
		) {
...
?>
					...
					<div class="form_field">
						...
						<?php echo $row['messaggio']; ?></textarea>
					</div>
					<div class="form_info">
					...
					<div class="form_submit">
						...
						<input type="hidden" name="id_messaggio"
...
if ($_REQUEST['op'] == 'delete') {
	...
	if ((int) $postData['padre'] == -1 && ($_SESSION['permessi'] >= ADMIN || $postData['autore'] == $_SESSION['login'])) {
		...
		$back = 'forum';
	} elseif ((int) $postData['padre'] != -1 && ($_SESSION['permessi'] >= ADMIN || $postData['autore'] == $_SESSION['login'])) {
...
if ($_REQUEST['op'] == 'read') {
	...
	if (!empty($row)) {
		...
		if (
			(($row['tipo'] == SOLORAZZA) && ($_SESSION['id_razza'] != $row['proprietari']) && ($_SESSION['permessi'] < ADMIN)) ||
			(($row['tipo'] == SOLOGILDA) && (strpos($_SESSION['gilda'],
				'*' . $row['proprietari'] . '*') === FALSE) && ($_SESSION['permessi'] < ADMIN)) ||
			(($row['tipo'] >= SOLOMASTERS) && ($_SESSION['permessi'] < MASTER)) ||
			(($row['tipo'] >= SOLOMODERATORS) && ($_SESSION['permessi'] < ADMIN))
		) {
				...
				<tr>
					...
					<td colspan="2">
						<div class="capitolo_elenco">
							<!--INSERISCI PER CENTRARE: style="text-align:center"-->
							<!--<img src="imgs/cerchisx.png" height="10px">-->
							<?php echo gdrcd_filter('out', $row['nome']); ?><img src="imgs/cerchidx.png" height="10px">
						</div>
					</td>
				</tr>
				<tr>
					<td colspan="2" class="forum_main_title">
						<div class="forum_post_title">
							...
							<!-- <img src="imgs/cerchisx.png" height="10px"><?php echo gdrcd_filter('out', $row['titolo']); ?><img src="imgs/cerchidx.png" height="10px"> -->
						</div>
					</td>
				</tr>
				<tr>
...
?>
						...
						<div class="forum_post_modify">
							<?php
			if ($chiuso == 0 || $_SESSION['permessi'] >= ADMIN) {
?>

							<a
								href="main.php?page=forum&op=composer&what=<?php echo gdrcd_filter('num', $_REQUEST['what']); ?>&where=<?php echo gdrcd_filter('num', $_REQUEST['where']); ?>&quote=<?php echo $row['id_messaggio']; ?>">[
								<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['link']['quote']); ?>]
							</a>
							<?php
			}

			if (($_SESSION['login'] == $row['autore'] && $chiuso == 0) || ($_SESSION['permessi'] >= ADMIN)) {
?>
							<a href="main.php?page=forum&op=modifica&what=<?php echo $row['id_messaggio']; ?>">[
								<?php echo $MESSAGE['interface']['forums']['link']['edit']; ?>]
							</a>
							<a
								href="main.php?page=forum&op=delete_conf&id_record=<?php echo $row['id_messaggio']; ?>&padre=<?php echo $row['id_messaggio_padre']; ?>">[
...
?>
				...
				if (($_SESSION['login'] == $row['autore'] && $row['chiuso'] == 0) || ($_SESSION['permessi'] >= ADMIN)) {
?>
							<a href="main.php?page=forum&op=modifica&what=<?php echo $row['id_messaggio']; ?>">[
								<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['link']['edit']); ?>]
							</a>
							<a
								href="main.php?page=forum&op=delete_conf&id_record=<?php echo $row['id_messaggio']; ?>&padre=<?php echo $row['id_messaggio_padre']; ?>">[
								<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['link']['delete']); ?>]
...
?>
			<a
				href="main.php?page=forum&op=composer&what=<?php echo gdrcd_filter('num', $_REQUEST['what']); ?>&where=<?php echo gdrcd_filter('num', $_REQUEST['where']); ?>">
...
<?php while ($row = gdrcd_query($result, 'fetch'))
		...
		<tr>
			...
			<td colspan="2">
				<div class="capitolo_elenco">
					<?php echo gdrcd_filter('out', $PARAMETERS['names']['forum']['plur'] . ' ' . strtolower($MESSAGE['interface']['forums']['type'][$ultimotipo])); ?><img
						src="imgs/cerchidx.png" height="10px">
				</div>
			</td>
		</tr>
		<?php } //if
...
?>
			...
			<td class="casella_elemento">
				<div class="elementi_elenco">
					...
					<a
						href="main.php?page=forum&op=visit&what=<?php echo gdrcd_filter('out', $row['id_araldo']); ?>&name=<?php echo gdrcd_filter('out', $row['nome']); ?>">
...
?>
	...
	while ($row = gdrcd_query($result, 'fetch')) {
		if ($row['tipo'] != $ultimotipo) {
			...
			if (
				($row['tipo'] != SOLORAZZA || $_SESSION['id_razza'] == $row['proprietari'] || $_SESSION['permessi'] >= CAPORAZZA) &&
				($row['tipo'] != SOLOGILDA || strpos($_SESSION['gilda'],
					'*' . $row['proprietari'] . '*') !== FALSE || $_SESSION['permessi'] >= CAPOGILDA) &&
				($row['tipo'] < SOLOMASTERS || $_SESSION['permessi'] >= MASTER) &&
				($row['tipo'] < SOLOMODERATORS || $_SESSION['permessi'] >= ADMIN)
			) {
?>
...
?>
										<div class="warning">
											<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['warning']['no_topic']); ?>
										</div>
										<?php
		} else {
?>
...
?>
													...
													<tr>
														...
														<td class="casella_titolo">
															<div class="capitolo_elenco">
																<?php echo gdrcd_filter('out', $MESSAGE['interface']['forums']['topic']['title']); ?><img
																	src="imgs/cerchidx.png" height="10px">
															</div>
														</td>
														<td class="casella_titolo">
													...
													<tr>
														...
														<td class="casella_elemento">
															<div class="elementi_elenco">
																<!-- Titolo -->
																<a
																	href="main.php?page=forum&op=read&what=<?php echo gdrcd_filter('out', $row['id_messaggio']); ?>&where=<?php echo gdrcd_filter('num', $_REQUEST['what']); ?>">
...
?>
														...
														<td class="casella_elemento">
															<div class="elementi_elenco">
																<!-- Autore -->
																<a
																	href="main.php?page=scheda&pg=<?php echo gdrcd_filter('out', $row['autore']); ?>">
																	<?php echo gdrcd_filter('out', $row['autore']); ?>
																</a>
															</div>
														</td>
...
-->
																...
																<div class="controllo_elenco">
																	<form
																		action="main.php?<?php echo $_SERVER['QUERY_STRING']; ?>"
																		method="post">
...
?>
											<a
												href="main.php?page=forum&op=visit&what=<?php echo gdrcd_filter('num', $_REQUEST['what']) ?>&offset=<?php echo $i; ?>">
												<?php echo $i + 1; ?>
											</a>
											<?php
				} else {
...
?>
										...
										<div class="link_back">
											<a
												href="main.php?page=forum&op=composer&what=-1&where=<?php echo gdrcd_filter('num', $_REQUEST['what']); ?>">
