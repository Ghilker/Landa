...
<?php
    ...
    if($_POST['op']=='insert')
    {
        ...
        if (gdrcd_query($thread, 'num_rows'))
        {
            ...
            if(($araldoData['tipo']==SOLORAZZA and ($_SESSION['id_razza']==$araldoData['proprietari'] || $_SESSION['permessi']>=CAPORAZZA)) ||
            ($araldoData['tipo']==SOLOGILDA and (strpos($_SESSION['gilda'],'*'.$araldoData['proprietari'].'*')!==FALSE || $_SESSION['permessi']>=CAPOGILDA)) ||
...
/*Restrizione di accesso i forum admin e master*/
	  	if ((($row['tipo']==SOLORAZZA)&&($_SESSION['id_razza']!=$row['proprietari'])&&($_SESSION['permessi']<ADMIN))||
    	(($row['tipo']==SOLOGILDA)&&(strpos($_SESSION['gilda'],'*'.$row['proprietari'].'*')===FALSE)&&($_SESSION['permessi']<ADMIN))||
...
<div class="panels_box">
    <table>
	<tr><!-- Intestazione tabella INTERNA-->
    <td colspan="2"><div class="capitolo_elenco"> <!--INSERISCI PER CENTRARE: style="text-align:center"-->
       <!--<img src="imgs/cerchisx.png" height="10px">--><?php echo gdrcd_filter('out',$row['nome']); ?><img src="imgs/cerchidx.png" height="10px">
    </div></td>
    </tr>
	<tr>
...
(($row['tipo']==SOLOMASTERS)&&($_SESSION['permessi']>=MASTER))||
($_SESSION['permessi']>=ADMIN)){ /*Restrizione di visualizzazione solo master e admin*/
...
<?php echo gdrcd_filter('out',$PARAMETERS['names']['forum']['plur'].' '.strtolower($MESSAGE['interface']['forums']['type'][$ultimotipo])); ?><img src="imgs/cerchidx.png" height="10px">
</div></td>
</tr>
<?php } //if
...
<?php 
	while($row = gdrcd_query($result, 'fetch'))
	{
  		if($row['tipo']!=$ultimotipo)
  		{
			...
			if (($row['tipo']!=SOLORAZZA || $_SESSION['id_razza']==$row['proprietari'] || $_SESSION['permessi']>=CAPORAZZA) &&
		    ($row['tipo']!=SOLOGILDA || strpos($_SESSION['gilda'],'*'.$row['proprietari'].'*')!==FALSE || $_SESSION['permessi']>=CAPOGILDA) &&
			($row['tipo']<SOLOMASTERS || $_SESSION['permessi']>=MASTER) &&
			($row['tipo']<SOLOMODERATORS || $_SESSION['permessi']>=ADMIN)) 
			{
?>
...
<tr><!-- Intestazione tabella ESTERNA -->
			<td colspan="2">
...
?>
  ...
  <tr><!-- Intestazione tabella -->
    <td class="casella_titolo"><div class="capitolo_elenco">
      <?php echo gdrcd_filter('out',$MESSAGE['interface']['forums']['topic']['title']); ?><img src="imgs/cerchidx.png" height="10px">
    </div></td>
    <td class="casella_titolo"><div class="capitolo_elenco">
