...
if ($_SESSION['permessi'] < ADMIN) {
	...
	<div class="page_title">
		<h2>
			<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['types']['page_name'][$types]); ?>
		</h2>
	</div>
...
?>
	...
	if ($_POST['op'] == 'modify') {
		...
		if ($types == "items") {
			$query = "UPDATE codtipooggetto SET descrizione ='" . gdrcd_filter('in', $_POST['nome']) . "' WHERE cod_tipo = " . gdrcd_filter('num', $_POST['id_record']) . " LIMIT 1";
		}
		if ($types == "guilds") {
...
?>
		...
		<div class="panels_box">
			<form action="main.php?page=gestione_tipi&types=<?php echo $types; ?>" method="post" class="form_gestione">

				<div class='form_label'>
					<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['types']['name']); ?>
				</div>
				<div class='form_field'>
					<input name="nome" value="<?php echo gdrcd_filter('out', $loaded_record['descrizione']); ?>" />
				</div>
...
<?php if ((isset($_POST['op']) === FALSE) && (isset($_REQUEST['op']) === FALSE)) {
		...
		<div class="elenco_record_gestione">
			<table>
				...
				<tr>
					...
					<td class="casella_titolo">
						<div class="titoli_elenco">
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['types']['code']); ?>
						</div>
					</td>
					<td class="casella_titolo">
						<div class="titoli_elenco">
				...
				<tr class="risultati_elenco_record_gestione">
					...
					<td class="casella_elemento">
						<div class="elementi_elenco">
							<?php echo gdrcd_filter('out', $$row['cod_tipo']); ?>
						</div>
					</td>
						...
						<div class="controlli_elenco">
							<div class="controllo_elenco">
								<form class="opzioni_elenco_record_gestione"
									action="main.php?page=gestione_tipi&types=<?php echo $types; ?>" method="post">
	 ...
	 <div class="pager">
       <?php if ($totaleresults > $PARAMETERS['settings']['records_per_page']) {
		...
		<a href="main.php?page=gestione_tipi&types=<?php echo $types; ?>&offset=<?php echo $i; ?>">
			<?php echo $i + 1; ?>
		</a>
		<?php } else {
					echo ' ' . ($i + 1) . ' ';
