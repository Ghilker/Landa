<div class="pagina_gestione_razze">
	...
	if ($_SESSION['permessi'] < MASTER) {
		...
		<div class="page_title">
			...
			<div style="margin-top: 15px; margin-bottom: 20px"><img src="/imgs/log_eventi.png"></div>
		</div>
							...
							<?php while ($row = gdrcd_query($result, 'fetch')) { ?>
								<tr class="risultati_elenco_record_gestione">
									...
									<td class="casella_elemento">
										<div class="elementi_elenco">
											<a
												href="main.php?page=scheda&pg=<?php echo gdrcd_filter('out', $row['nome_interessato']); ?>">
				...
				<div class="pager">
					...
					for ($i = 0; $i <= floor($totaleresults / $PARAMETERS['settings']['records_per_page']); $i++) {
						if ($i != $_REQUEST['offset']) { ?>
								<a
									href="main.php?page=log_eventi&op=view&which_log=<?php echo $_REQUEST['which_log']; ?>&offset=<?php echo $i; ?>">
									<?php echo $i + 1; ?>
								</a>
								<?php } else {
							echo ' ' . ($i + 1) . ' ';
