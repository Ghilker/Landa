<div class="pagina_schedam_odifica">
	    ...
	    if ($confirm_updating) {
		    ...
		    if (isset($_POST['op']) === TRUE) {
			    ...
			    if ((gdrcd_filter('get', $_REQUEST['pg']) == $_SESSION['login']) && (gdrcd_filter('get', $_POST['op']) == 'modify')) {
				    ...
				    /*MODIFICA DI JOLLANZA PER PRESTAVOLTO*/
				    gdrcd_query("UPDATE personaggio SET cognome = '" . gdrcd_filter('in', $_POST['modifica_cognome']) . "', affetti = '" . $modifica_affetti . "', descrizione = '" . $modifica_background . "', url_media = '" . gdrcd_filter('in', $_POST['modifica_url_media']) . "', blocca_media = " . (int) $blocca_media . ", url_img = '" . gdrcd_filter('in', $_POST['modifica_url_img']) . "', url_img_chat = '" . gdrcd_filter('in', $_POST['modifica_url_img_chat']) . "', prestavolto = '" . gdrcd_filter('in', $_POST['modifica_prestavolto']) . "', online_status = '" . $online_state . "' WHERE nome = '" . gdrcd_filter('in', $_REQUEST['pg']) . "'");
				    /*FINE MODIFICA*/

				    /*MODIFICA DI JOLLANZA PER PRESTAVOLTO
					gdrcd_query("UPDATE personaggio SET cognome = '".gdrcd_filter('in',$_POST['modifica_cognome'])."', affetti = '".$modifica_affetti."', descrizione = '".$modifica_background."', url_media = '".gdrcd_filter('in',gdrcd_filter('fullurl',$_POST['modifica_url_media']))."', blocca_media = ".(int)$blocca_media.", url_img = '".gdrcd_filter('in',gdrcd_filter('fullurl',$_POST['modifica_url_img']))."', url_img_chat = '".gdrcd_filter('in',gdrcd_filter('fullurl',$_POST['modifica_url_img_chat']))."', prestavolto = '".gdrcd_filter('in',$_POST['modifica_prestavolto'])."', online_status = '".$online_state."' WHERE nome = '".gdrcd_filter('in',$_REQUEST['pg'])."'");
					FINE MODIFICA*/
				    ...
				    /* PERMESSI MODIFICA PRESTAVOLTO */
				    /*Se un master o superiore ha richiesto di modificare il prestavolto del pg*/
			    } elseif (($_SESSION['permessi'] >= USER) && (gdrcd_filter('get', $_POST['op']) == 'modify_prestavolto')) {
				    gdrcd_query("UPDATE personaggio SET prestavolto = '" . gdrcd_filter('in', $_POST['modifica_prestavolto']) . "', prestavolto = " . gdrcd_filter('num', $_POST['modifica_prestavolto']) . " WHERE nome = '" . gdrcd_filter('in', $_REQUEST['pg']) . "'");

				    echo '<div class="warning">' . gdrcd_filter('out', $MESSAGE['warning']['modified']) . '</div>';


				    /*Se un master o superiore ha richiesto di modificare lo status del pg - SEGNI PARTICOLARI*/
			    } elseif (($_SESSION['permessi'] >= MASTER) && (gdrcd_filter('get', $_POST['op']) == 'modify_status')) {
				    ...
				    gdrcd_query("UPDATE personaggio SET stato = '" . gdrcd_filter('in', $_POST['modifica_status']) . "', stato2 = '" . gdrcd_filter('in', $_POST['modifica_status2']) . "', salute = " . gdrcd_filter('num', $_POST['modifica_salute']) . " WHERE nome = '" . gdrcd_filter('in', $_REQUEST['pg']) . "'");
			    ...
			    } elseif (($_SESSION['permessi'] >= MASTER) && (gdrcd_filter('get', $_POST['op']) == 'exile')) {
			    ...
			    $record = gdrcd_query("SELECT descrizione, affetti, cognome, prestavolto, online_status, url_img, url_img_chat, url_media, blocca_media, stato, stato2, salute FROM personaggio WHERE nome='" . gdrcd_filter('get', $_REQUEST['pg']) . "'");
		    }
	...
	<div class="page_title">
		<h2>
			<?php echo gdrcd_filter('out', $MESSAGE['interface']['sheet']['page_name']); ?>
		</h2>
	</div>
				...
				<form action="main.php?page=scheda_modifica" method="post">
					...
					<div class='form_field'>
						<input type="text" name="modifica_cognome"
							value="<?php echo gdrcd_filter('out', $record['cognome']); ?>" class="form_input" />
					</div>
					...
					<!-- MODIFICA JOLLANZA PER PRESTAVOLTO -->
					<div class='form_label'>
						Nome del prestavolto
					</div>
					<div class='form_field'>
						<input type="text" name="modifica_prestavolto"
							value="<?php echo gdrcd_filter('out', $record['prestavolto']); ?>" class="form_input" />
					</div>
					<!-- FINE MODIFICA PRESTAVOLTO -->
			    ...
			    if ($PARAMETERS['mode']['chat_avatar'] == 'ON') {
					...
					<div class='form_field'>
						<input type="text" name="modifica_url_img_chat"
							value="<?php echo gdrcd_filter('out', $record['url_img_chat']); ?>" class="form_input" />
					</div>
					<?php
			    }
                    ?>
			    ...
			    if ($PARAMETERS['mode']['user_online_state'] == 'ON') {
					...
					<div class='form_field'>
						<input type="text" name="online_state" class="form_input"
							value="<?php echo gdrcd_filter('out', $record['online_status']); ?>" maxlength="100" />
					</div>
					<?php
			    }
                    ?>
					...
					<div class='form_field'>
						<textarea type="textbox" name="modifica_background"
							class="form_textarea"><?php echo gdrcd_filter('out', $record['descrizione']); ?></textarea>
					</div>
					<div class="form_info">
					...
					<div class='form_field'>
						<textarea type="textbox" name="modifica_affetti"
							class="form_textarea"><?php echo gdrcd_filter('out', $record['affetti']); ?></textarea>
					</div>
					<div class="form_info">
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['help']['bbcode']); ?>
					</div>
					...
					<div class='form_field'>
						<input type="checkbox" name="blocca_media" <?php echo ($record['blocca_media']) ? 
			    	'checked="checked"' : ''; ?> class="form_input" />
					</div>
				...
				<form action="main.php?page=scheda_modifica" method="post">
					...
					<div class='form_field'>
						<textarea type="textbox" name="modifica_status"
							class="form_textarea"><?php echo gdrcd_filter('out', $record['stato']); ?></textarea>
					</div>
					...
					<div class='form_field'>
						<textarea type="textbox" name="modifica_status2"
							class="form_textarea"><?php echo gdrcd_filter('out', $record['stato2']); ?></textarea>
					</div>
