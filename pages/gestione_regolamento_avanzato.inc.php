<div class="pagina_gestione_abilita">
	...
	if ($_SESSION['permessi'] < GESTORE) {
		...
		<div class="page_title">
			<!--<h2><?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['page_name']); ?></h2>-->
		...
		<div class="page_body">
			...
			if ($_POST['op'] == 'insert') {
				...
				if (is_numeric($_POST['articolo']) == TRUE) {
					gdrcd_query("INSERT INTO regolamento_avanzato (articolo, titolo, testo) VALUES (" . gdrcd_filter('num', $_POST['articolo']) . ", '" . gdrcd_filter('in', $_POST['titolo']) . "', '" . gdrcd_filter('in', $_POST['testo']) . "')");
				...
				<div class="link_back">
					<a href="main.php?page=gestione_regolamento_avanzato">
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['link']['back']); ?>
					</a>
				</div>
				<?php } ?>
			...
			if ($_POST['op'] == 'erase') {
				...
				gdrcd_query("DELETE FROM regolamento_avanzato WHERE articolo=" . gdrcd_filter('num', $_POST['id_record']) . " LIMIT 1");
				?>
				<div class="warning">
			...
			if ($_POST['op'] == 'doedit') {
				...
				if ((is_numeric($_POST['art']) == TRUE) && (is_numeric($_POST['articolo']) == TRUE)) {
					...
					gdrcd_query("UPDATE regolamento_avanzato SET titolo ='" . gdrcd_filter('in', $_POST['titolo']) . "', testo ='" . gdrcd_filter('in', $_POST['testo']) . "', articolo = " . gdrcd_filter('num', $_POST['articolo']) . " WHERE articolo = " . gdrcd_filter('num', $_POST['art']) . " LIMIT 1");
			...
			if (
				...
				if ($_POST['op'] == 'edit') {
					...
					$loaded_record = gdrcd_query("SELECT * FROM regolamento_avanzato WHERE articolo=" . gdrcd_filter('num', $_POST['id_record']) . " LIMIT 1 ");
				...
				<div class="panels_box">
					<form action="main.php?page=gestione_regolamento_avanzato" method="post" class="form_gestione">

						<div class='form_label'>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['art']); ?>
						</div>
						...
						<div class='form_label'>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['title']); ?>
						</div>
						<div class='form_field'>
						...
						<div class='form_label'>
							<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['infos']); ?>
						</div>
						<div class='form_field'>
			...
			$record_globale = gdrcd_query("SELECT COUNT(*) FROM regolamento_avanzato");
			$totaleresults = $record_globale['COUNT(*)'];
			//Lettura record
			$result = gdrcd_query("SELECT articolo, titolo, testo FROM regolamento_avanzato ORDER BY articolo LIMIT " . $pagebegin . ", " . $pageend . "", 'result');
			...
			if ($numresults > 0) { ?>
					...
					<div class="elenco_record_gestione">
						<table>
							...
							<tr>
								...
								<td class="casella_titolo">
									<div class="titoli_elenco"><?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['titolo']); ?></div>
								</td>
								<td class="casella_titolo">
									<div class="titoli_elenco">
									...
									<td class="casella_controlli"><!-- Iconcine dei controlli -->
										...
										<div class="controlli_elenco">
											<div class="controllo_elenco">
												<form class="opzioni_elenco_record_gestione" action="main.php?page=gestione_regolamento_avanzato"
													method="post">
				...
				<div class="pager">
					...
					for ($i = 0; $i <= floor($totaleresults / $PARAMETERS['settings']['records_per_page']); $i++) {
						if ($i != gdrcd_filter('num', $_REQUEST['offset'])) { ?>
								<a href="main.php?page=gestione_regolamento_avanzato&offset=<?php echo $i; ?>">
									<?php echo $i + 1; ?>
								</a>
								<?php } else {
							echo ' ' . ($i + 1) . ' ';
				...
				<!-- link crea nuovo -->
				<div class="link_back">
					<a href="main.php?page=gestione_regolamento_avanzato&op=new">
						<?php echo gdrcd_filter('out', $MESSAGE['interface']['administration']['adv_rules']['link']['new']); ?>
					</a>
				</div>
