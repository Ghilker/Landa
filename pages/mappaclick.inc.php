...
if (
    (isset($_POST['op']) === true) && (($_POST['op'] == gdrcd_filter('out', $MESSAGE['interface']['maps']['leave'])) ||
...
if (gdrcd_query($result, 'num_rows') == 0) {
    ...
    while ($row = gdrcd_query($result, 'fetch')) {
        ...
        if ($echoed_title === false) {
            ...
            /*echo '<h2>'.$row['nome_mappa'].'</h2>'; --> ORIGINALE*/
    ...
    if ($vicinato != INVIAGGIO) { ?>
        <!--SCRITTA ALTRE ZONE: <div class="page_title">
                    <h2><?php echo gdrcd_filter('out', $MESSAGE['interface']['maps']['more']); ?></h2>
                    </div>-->
        <div class="mappaclick_more">
            <!--<?php /* Carico le mappe dell'eventuale vicinato - ABILITA PULSANTINI IN BASSO */
