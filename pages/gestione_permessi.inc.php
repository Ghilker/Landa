...
<?php /*Modifica di un record*/
	  if ((gdrcd_filter('',$_POST['op'])==$MESSAGE['interface']['administration']['roles']['submit']['edit']) || 
		 ...
		 switch(gdrcd_filter('num',$_POST['permessi'])){
            ...
            case CAPORAZZA:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['race']['lead']);break;}
			case GUIDA:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['guida']['sing']);break;}
            case CAPOGILDA:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['guild_name']['lead']);break;}
			case MASTER:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['master']['sing']);break;}
            case MODERATORE:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['moderator']['sing']);break;} /*aggiunto io + GESTORE era GESTIONE*/
            case ADMIN:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['admin']['sing']);break;}
   			case GESTORE:{$newrole = gdrcd_filter('out',$PARAMETERS['names']['administrator']['sing']);break;}
		 }
