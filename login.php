...
if (!empty($record) and gdrcd_password_check($pass1,$record['pass']) && ($record['permessi']>-1) && (strtotime($record['ora_entrata']) < strtotime($record['ora_uscita'])||(strtotime($record['ultimo_refresh'])+300) < time()))
{
	...
	*/
	...
	{
		$_SESSION['mappa']	= $record['ultima_mappa'] = 1;
        /*$_SESSION['mappa']	= $record['ultima_mappa']; --> ORIGINALE LOGIN ALL'ULTIMA MAPPA*/
	}
...
else 
{
	...
	if (($login1 != '') && ($pass1 != ''))
	{
		...
		gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento, descrizione_evento) VALUES ('".gdrcd_filter('in',$_SESSION['login'])."', tizio, NOW(), ".ERRORELOGIN." ,'".$_SERVER['REMOTE_ADDR']."')");
		...
		if ($iErrorsNumber>=10)
		{
			gdrcd_query("INSERT INTO blacklist (ip, nota, ora, host) VALUES ('".$_SERVER['REMOTE_ADDR']."', '".$login1." (tenta password)', NOW(), '".((strlen($host) > 20) ? substr($host,0,20) : $host)."')");
		}
	}
}
